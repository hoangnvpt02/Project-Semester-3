      ...
      <div class="flex flex-wrap">
        ...
        <div class="w-full lg:w-8/12 px-4">
          <div class="relative w-full mb-3">
            ...
            <textarea type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" rows="4" required v-model="medicine.note" name="note">
            </textarea>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    getCarById(id) {
      MedicineService.getById(id)
        .then((response) => {
          ...
          this.medicine.note = response.data.note;
        })
        .catch((e) => {
          console.log(e);
        });
    ...
    updateCar() {
      ...
      MedicineService.update(this.$route.params.id, data)
        .then(() => {
